...
**Vista do carrinho — Cupom ou Tabela:** com a venda em andamento, o cabeçalho "Cupom em andamento" traz um botão que alterna como os itens são exibidos:
- **Cupom** (padrão) — visual de cupom térmico (papel, fonte monoespaçada, picotado), parecido com o que será impresso.
- **Tabela** — formato convencional em lista/colunas (**# · Produto · Qtd · Unitário · Total**), no tema do app, com os botões **−/+** e **remover** sempre visíveis em cada linha.

Os dois mostram os mesmos itens e o mesmo total — muda só a aparência. É só preferência de quem opera; **não afeta o cupom impresso**. A escolha fica salva no computador (por operador) e pode ser trocada a qualquer momento, inclusive no meio da venda.
...
- **UPC-A × EAN-13 (zero à esquerda):** muitos leitores enviam o código no formato **UPC-A (12 dígitos)**, enquanto o produto pode ter sido cadastrado em **EAN-13 (13 dígitos, com um zero à esquerda)** — ou o contrário. O PDV agora **casa os dois formatos automaticamente**, ignorando o zero à esquerda, então o bipe encontra o produto mesmo que o cadastro esteja no outro formato. Importante: isso **não conserta um código digitado errado/incompleto**. A forma mais segura de cadastrar é **bipar o produto direto no campo de código de barras** na tela de cadastro (em vez de digitar) — assim o que fica salvo é exatamente o que o leitor envia.

**Aviso de item que não entrou (trava de segurança):** sempre que um bipe **não adiciona o produto à cestinha** — porque está **esgotado** (estoque zero ou insuficiente) ou porque o **código não está cadastrado** — a tela mostra um **alerta vermelho grande no topo** ("⛔ Produto bloqueado" ou "⛔ Produto não encontrado") acompanhado de um **bip de erro** bem diferente do bip de sucesso do leitor, para que ninguém deixe o item passar sem perceber. O alerta some sozinho em poucos segundos (ou ao tocar nele) e **não atrapalha a próxima bipagem**. A barra de pesquisa também é **limpa automaticamente** nesses casos, evitando que bipes repetidos acumulem códigos no campo.
