...
type TipoRecorrencia = "PARCELADA" | "RECORRENTE" | "UNICA" | string;
...
const TONES_LIST = ["c1", "c2", "c3", "c4", "c5"] as const;

function hashTone(s?: string | null): string {
  if (!s) return "c1";
  let h = 0;
...
function diasDiff(iso?: string | null): number {
  if (!iso) return 0;
...
function statusUI(c: Conta): Bill["status"] {
...
function dueStateUI(c: Conta): Bill["dueState"] {
...
function dueRel(c: Conta, ehPagar: boolean): string {
  if (c.status === "PAGA") {
...
function splitVal(n: number): { value: string; cents: string } {
  const { amount, cents } = splitValor(n);
  return { value: amount, cents };
}

function calcularKpis(contas: Conta[], ehPagar: boolean): KpiData[] {
  ...
  const pct = (n: number) => totGeral > 0 ? Math.round((n / totGeral) * 100) : 0;
  ...
  const mk = (
    ...
    tone,
    ...splitVal(val),
    footLeft: `${qtd} ${qtd === 1 ? "conta" : "contas"}`,
    progress: pct(val),
  });

  return [
    mk("pendentes", "Pendentes",         "clock",    "amber",   totPend, qPend),
    mk("atrasadas", "Atrasadas",         "alert",    "coral",   totAtr,  qAtr),
    mk("vencendo",  "Vencendo em 7 dias","calendar", "iris",    totProx, qProx),
    mk("pagas",     ehPagar ? "Pagas" : "Recebidas", "check", "emerald", totPago, qPago),
  ];
}

export default function FinanceiroPage({ user }: FinanceiroPageProps) {
  const podeEditar = user?.role === "ADMIN" || user?.role === "GERENTE";
  const [aba, setAba] = useState<string>("pagar");
  const [contagens, setContagens] = useState<{ pagar: number | null; receber: number | null }>({
    pagar: null,
    receber: null,
  });

  function setCount(tipo: TipoConta, n: number) {
...
interface FinanceiroTabsProps {
  ...
  podeEditar: boolean;
  contagens: { pagar: number | null; receber: number | null };
  setCount: (tipo: TipoConta, n: number) => void;
}

function FinanceiroTabs({ aba, setAba, podeEditar, contagens, setCount }: FinanceiroTabsProps) {
  const tabs: TabDef[] = [
  ...
  const novoTipo: TipoConta = aba === "receber" ? "receber" : "pagar";
...
interface ContaModalReloaderProps {
  tipo: TipoConta;
  onFechar: () => void;
}

function ContaModalReloader({ tipo, onFechar }: ContaModalReloaderProps) {
...
function ContasView({ tipo, podeEditar, onContas }: ContasViewProps) {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const lista = Array.isArray(data) ? (data as Conta[]) : [];
      setContas(lista);
      onContas?.(lista);
    } catch (err) {
      setErro((err as Error).message || "Erro ao carregar");
    } finally {
      setCarregando(false);
    }
  ...
  useEffect(() => {
    function onReload(e: Event) {
      const detail = (e as CustomEvent).detail as { tipo?: TipoConta } | undefined;
      if (!detail || detail.tipo === tipo) carregar();
    }
  ...
  async function executarReabrir(bill: Conta) {
  ...
  async function executarCancelar(bill: Conta) {
...
function useContasFinanceiro() {
  ...
  const recarregar = useCallback(async () => {
    ...
    try {
      ...
      setContasPagar(Array.isArray(pagar) ? (pagar as Conta[]) : []);
      setContasReceber(Array.isArray(receber) ? (receber as Conta[]) : []);
    } catch (err) {
      setErro((err as Error).message || "Erro ao carregar contas");
    } finally {
      setCarregando(false);
    }
  }, []);
...
function FluxoView() {
  ...
  const linhas = useMemo<FluxoLinha[]>(() => {
    const items: Omit<FluxoLinha, "saldo">[] = [];
...
function ConcilView() {
  ...
  const grupos = useMemo<GrupoConcil[]>(() => {
    const map = new Map<string, GrupoConcil>();
    function add(c: Conta, tipo: TipoConta) {
...
type SimplesTone = "emerald" | "coral" | "iris" | "amber";
...
function KpiCardSimples({ label, value, foot, tone = "iris" }: KpiCardSimplesProps) {
  const cor: Record<SimplesTone, string> = {
  ...
  return (
    <div className="bg-surface border border-hairline-soft rounded-card shadow-card p-[18px_20px]">
      ...
      <div className={`font-mono text-[24px] font-medium tracking-[-0.02em] ${cor[tone]}`}>{value}</div>
      <div className="text-fg-faint text-xs mt-2">{foot}</div>
    </div>
  );
}

interface ThSimpleProps {
  children: React.ReactNode;
  align?: "left" | "right" | "center";
  last?: boolean;
}

function ThSimple({ children, align = "left", last }: ThSimpleProps) {
  return (
    <th
